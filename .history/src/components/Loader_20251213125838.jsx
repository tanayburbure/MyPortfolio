...
function Loader({ setLoading }) {
  ...
  const [assetsReady, setAssetsReady] = useState(false); // ✅ NEW

  /* ---------------- PRELOAD ASSETS (SILENT) ---------------- */
  useEffect(() => {
    const preload = async () => {
      try {
        await Promise.all([
  ...
  const handleStartClick = () => {
    // ⛔ Do NOT exit until assets are ready
  ...
  return (
    <div className={`h-screen flex items-center justify-center fixed inset-0 bg-[#0D0D0D] z-50 ${hideLoader ? 'hidden' : ''}`}>
      <style>{`
