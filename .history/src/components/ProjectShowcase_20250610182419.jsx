...
function Descriptions({ projects, selectedProject, isMobile }) {
    return (
        <div className="absolute top-[-2.9vh] mt-[3.40vh] w-screen z-2 pointer-events-none">
            {projects.map((project, index) => (
                <div
                    key={index}
                    className="bg-[#ec4e39] h-[7.4vh] md:h-[6.8vw] pt-2 text-[#0D0D0D] font-thin flex flex-col md:flex-row justify-between items-center px-[5%] md:px-[15%] transition-all duration-500 ease-in-out"
                    style={{
                        clipPath: selectedProject === index ? "inset(0 0 0)" : "inset(50% 0 50%)",
                    }}
                >
