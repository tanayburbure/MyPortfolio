...
function Motto() {
  ...
  useEffect(() => {
    if (isMobile) {
      setOffsetY(0); // Reset to 0 when mobile
      return; // Exit - no effect on mobile
    }
    ...
    const handleScroll = () => {
      ...
      requestId = requestAnimationFrame(() => {
        setOffsetY(window.scrollY * 0.02); // Original desktop parallax
        requestId = null;
      });
    };
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden">
      {/* Background - fixed on desktop, static on mobile */}
      <div
        className={`${isMobile ? 'absolute' : 'fixed'} inset-0 bg-cover bg-center`}
        style={{
          backgroundImage: "url('./images/sukuna4.jpg')",
          opacity: 0.6,
          transform: isMobile ? 'translateY(0)' : `translateY(${offsetY}px)`,
