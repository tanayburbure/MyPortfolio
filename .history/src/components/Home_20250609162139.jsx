...
function Home() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      animationFrameId = requestAnimationFrame(() => {
        ...
        // Start with a small initial offset to prevent top gap
        const initialOffset = isMobile ? 20 : 0;
        setOffsetY(initialOffset + (window.scrollY * parallaxFactor));
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden">
      {/* Background Image with enhanced mobile handling */}
      <div
        className="absolute inset-0 bg-fixed bg-center bg-no-repeat bg-cover z-0"
        style={{
          backgroundImage: "url('./images/new.jpg')",
          backgroundPositionY: `calc(100% + ${offsetY}px)`, // Changed from 110% to 100%
          opacity: 0.4,
