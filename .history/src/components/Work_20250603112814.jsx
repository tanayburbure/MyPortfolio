...
function Title({ project, index, setSelectedProject }) {
    ...
    return (
      <div className="relative">
        ...
        >
          {/* Title wrapper to align both texts perfectly */}
          <div className="pl-[15%] p-2 relative h-fit">
            {/* Base text (dark) */}
            <p className="text-[#2F2D29] text-[8vw] leading-[6.5vw] font-medium relative z-0">
              {project.title}
            </p>
  
            {/* Overlay text (light reveal) */}
            <motion.p
              style={{ clipPath: clipPathStyle }}
              className="text-[8vw] leading-[6.5vw] font-medium absolute top-0 left-0 text-[#B7AB98] z-10 pointer-events-none"
            >
              {project.title}
            </motion.p>
          </div>
    ...
    <div className="relative">
      ...
      >
        <div className="pl-[15%] p-2 relative">
          {/* Base Text (Dark color) */}
          <p className="text-[#2F2D29] text-[8vw] leading-[6.5vw] font-medium relative z-0">
            {project.title}
          </p>

          {/* Reveal Text (Light color) */}
          <motion.p
            style={{
