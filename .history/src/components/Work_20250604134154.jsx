...
function Title({ project, index, setSelectedProject, containerRef }) {
  ...
  const { scrollYProgress } = useScroll({
    ...
    layoutEffect: false, // avoids hydration mismatch
  });
  ...
  return (
    <div className="absolute top-[-3vh] mt-[2.85vh] w-screen z-20 pointer-events-none">
      {projects.map((project, index) => (
        <motion.div
          key={index}
          className="bg-[#ec4e39] h-[6.6vw] text-[#0D0D0D] font-medium flex justify-between items-center px-[15%] absolute w-full"
          initial={false}
          animate={{
            clipPath:
