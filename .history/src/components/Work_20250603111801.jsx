...
function Title({ project, index, setSelectedProject }) {
    ...
    return (
        <div className="relative">
            ...
            >
                <div className="inline-block text-[8vw] leading-[6.5vw] font-medium relative pl-[15%]">
                    {/* Background layer (light text) */}
                    <p className="text-[#b7ab98] absolute top-0 left-0 z-0 select-none pointer-events-none">
                        {project.title}
                    </p>

                    {/* Foreground layer (dark text revealed via scroll) */}
                    <motion.p
                        style={{ clipPath: clipPathStyle }}
                        className="text-[#2F2D29] relative z-10 p-2"
                    >
                        {project.title}
                    </motion.p>
                </div>
