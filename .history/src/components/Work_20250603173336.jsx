...
function Descriptions({ projects, selectedProject }) {
    return (
        <div className="absolute top-[-0.4vh] mt-2 w-screen z-2 pointer-events-none b-[0.9vh]">
            {projects.map((project, index) => (
                <div
                    key={index}
                    className="bg-[#ec4e39] h-[6.4vw]  text-[#0D0D0D] font-medium flex justify-between items-center px-[15.5%] transition-all duration-500 ease-in-out"
                    style={{
                        clipPath: selectedProject === index ? "inset(0 0 0)" : "inset(50% 0 50%)"
                    }}
                >
                    <p className="text-[#010101] font-medium text-[7.7vw] relative z-1">
