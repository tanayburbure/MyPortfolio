    ...
    const interval = setInterval(() => {
      setPercentage((prev) => {
        if (prev < 100) {
          ...
          setTimeout(() => setHideTimer(true), 300); // Delay hiding by 2 seconds
          return 100;
        }
      });
  ...
  return (
    <div style={styles.container}>
      ...
      <img
        ...
        alt="Animated Logo"
      />
      <button className={`border-[1px] ${hideTimer ? 'block moveUp' : 'hidden'} text-[2vh] absolute top-[57vh] px-8 py-[1px] pb-1 rounded-full border-[#b7ab98] text-[#b7ab98]`}>
        S T A R T
      </button>
    </div>
  );
