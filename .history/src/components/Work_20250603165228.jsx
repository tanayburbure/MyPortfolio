...
function Descriptions({ projects, selectedProject }) {
    return (
        <div className="absolute -top-1 h-full w-screen  z-2 pointer-events-none">
            {projects.map((project, index) => (
                <div
                    key={index}
                    className="bg-[#ec4e39] text-[#0D0D0D] font-medium flex justify-between items-center px-[15.5%] transition-all duration-500 ease-in-out"
                    style={{
