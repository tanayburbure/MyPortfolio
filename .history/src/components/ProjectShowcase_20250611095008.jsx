...
function Title({ project, index, setSelectedProject, isMobile }) {
    ...
    const handleHoverIn = () => setSelectedProject(index);
    const handleHoverOut = () => setSelectedProject(null);
    ...
    return (
        <div className="relative">
            <div
                ...
                ref={containerRef}
            >
                <div className="inline-block md:pl-[1vw] lg:pl-[14.5vw] relative">
                    <p
                        ...
                        onTouchEnd={() => setTimeout(handleHoverOut, 1000)}
                    >
                        {project.title}
                    </p>

                    {/* ✅ Overlay title ONLY on desktop */}
                    {!isMobile && (
                        <motion.p
                            style={{ clipPath: clipPathStyle }}
                            className="absolute top-0 sm:left-[12vw] md:left-[1vw] lg:left-[14.5vw] text-[#b7ab98] font-thin text-[6vh] sm:text-[5.5vw] md:text-[6vw] lg:text-[8.2vw] leading-[6.5vh] sm:leading-[5.5vw] md:leading-[5.8vw] lg:leading-[6.5vw] pointer-events-none"
                        >
                            {project.title}
                        </motion.p>
                    )}
    ...
    return (
        <div className="absolute top-[-3vh] mt-[3.20vh] w-screen z-2 pointer-events-none">
            {projects.map((project, index) => (
                <div
                    key={index}
                    className="bg-[#ec4e39] h-[6.4vh] sm:h-[7vh] md:h-[5.5vw] lg:h-[6.8vw] pt-2 text-[#0D0D0D] font-thin flex flex-col sm:flex-row justify-between items-center px-[5%] sm:px-[10%] md:px-[12%] lg:px-[14.8%] transition-all duration-500 ease-in-out"
                    style={{
                        clipPath: selectedProject === index ? "inset(0 0 0)" : "inset(50% 0 50%)",
                    }}
                >
