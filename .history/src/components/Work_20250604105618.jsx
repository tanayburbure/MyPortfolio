...
export default function Work() {
  ...
  return (
    <>
      ...
      {/* Added relative position to container */}
...
function Title({ project, index, setSelectedProject }) {
  ...
  return (
    // Added relative position to this container
...
function Descriptions({ projects, selectedProject }) {
  return (
    // Added transform style to ensure proper stacking context
    <div className="absolute top-[-3vh] mt-[2.85vh] w-screen z-20 pointer-events-none transform">
      {projects.map((project, index) => (
        <motion.div
          key={index}
          className="bg-[#ec4e39] h-[6.6vw] text-[#0D0D0D] font-medium flex justify-between items-center px-[15%]"
