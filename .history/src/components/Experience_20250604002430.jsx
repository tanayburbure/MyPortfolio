...
function Experience() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      gsap.to(imageRef.current, {
        y: -100, // upward movement (adjust as needed)
        ease: 'none',
        scrollTrigger: {
  ...
  return (
    <div className='h-screen pl-[31vh] pb-4 flex flex-col justify-center overflow-hidden'>
      <div className='absolute top-0 left-0 w-full h-full -z-10 overflow-hidden'>
        <img
          ref={imageRef}
          className='w-full h-full object-cover'
          src={`public\images\duck.avif`}
          alt='Experience'
        />
      </div>
      ...
      <div ref={textRef}>
        <h2 className='text-[9vh] tracking-tighter w-[77%] font-semibold leading-[4.5vw] text-transparent bg-gradient-to-r from-[#212121] to-[#B7AB98] bg-[length:200%_100%] bg-right bg-clip-text [-webkit-background-clip:text]'>
          I&apos;m a <span className='text-[#EB5939]'>visually skilled</span> interactive web
          designer and developer with a strong focus on crafting high-quality, impactful digital
          experiences.
        </h2>
      </div>
      ...
      <style jsx global>{`
        ...
        .span-line {
          display: block;
          background: linear-gradient(to right, #EB5939 50%, #212121 50%);
          background-size: 200% 100%;
          background-position-x: 100%;
          color: transparent;
          background-clip: text;
