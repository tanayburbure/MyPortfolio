    ...
    return (
        <div className="absolute top-0 w-screen z-2 pointer-events-none">
            {projects.map((project, index) => (
                <div
                    key={index}
                    className="bg-[#ec4e39] h-[7vw] pb-[0.9vh] text-[#0D0D0D] font-medium flex justify-between items-center px-[15.5%] transition-all duration-500 ease-in-out"
                    style={{
                        clipPath: selectedProject === index ? "inset(0 0 0)" : "inset(50% 0 50%)"
                    }}
                >
                    <p className="text-[#010101] mb-6 font-medium text-[8vw] relative z-1">
