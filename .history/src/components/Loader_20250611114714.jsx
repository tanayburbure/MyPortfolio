...
function Loader({ setLoading }) {
  ...
  return (
    <div className={`h-screen flex items-center justify-center fixed top-0 left-0 w-full bg-[#0D0D0D] z-50 ${hideLoader ? 'hidden' : ''}`}>
      ...
      {/* Percentage Display */}
      {!hideTimer && (
        <div className="absolute top-[36vh] text-white pr-[4px] text-[3.5vw] sm:text-[1.8vh]">
          {percentage}%
        </div>
      )}

      {/* SVG Circle */}
      <svg className="w-[40vw] h-[40vw] sm:w-[16vw] sm:h-[16vw]" viewBox="0 0 64 64">
        <circle
          className="circle"
          cx="32"
          cy="32"
      ...
      <button
        ...
        } text-[4vw] sm:text-[2vh] absolute top-[60vh] sm:top-[55.5vh] px-8 sm:px-12 py-[6px] sm:py-[8px] pb-[6px] sm:pb-2 rounded-full border-[#EB5939] text-[#EB5939] hover:bg-[#EB5939] hover:text-[#0D0D0D] hover:border-[#EB5939] transition-all duration-500 ease-in-out`}
        aria-label="Start"
