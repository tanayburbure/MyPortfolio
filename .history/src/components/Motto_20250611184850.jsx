...
function Motto() {
  ...
  useEffect(() => {
    if (isMobile) return; // NO EFFECTS ON MOBILE

    const handleScroll = () => {
      setOffsetY(window.scrollY * 0.02); // Original desktop effect
    };
  ...
  return (
    <div className="relative w-full h-screen">
      {/* 3. Background - ABSOLUTE POSITION (never fixed) */}
      <div
        className="absolute inset-0 bg-cover bg-center"
        style={{
          backgroundImage: "url('./images/sukuna4.jpg')",
          opacity: 0.6,
          // 4. Only apply transform on desktop
