...
function Title({ project, index, setSelectedProject }) {
    ...
    return (
        <div className="relative">
            <div
                className="border-b border-[#b7ab9820] no-mask font-[font13]"
                ref={containerRef}
            >
                <div className="inline-block pl-[15%] relative ">
                    <p
                        className="text-[#2F2D29] pb-[1vh]  font-medium text-[7.5vw] leading-[6.5vw] cursor-pointer"
                    ...
                    <motion.p
                        ...
                        className="absolute top-0 left-[14.85vw] text-[#b7ab98] pb-[1vh]  font-medium text-[8.2vw] leading-[6.5vw] pointer-events-none"
                    >
                        {project.title}
                    </motion.p>
                </div>
...
function Descriptions({ projects, selectedProject }) {
    return (
        <div className="absolute top-[-3vh] mt-[2.85vh] w-screen z-2 pointer-events-none font-[font13]">
            {projects.map((project, index) => (
                <div
                    key={index}
                    className="bg-[#ec4e39] h-[6.6vw] text-[#0D0D0D] font-medium flex justify-between items-center px-[14.8%] transition-all duration-500 ease-in-out"
                    style={{
                        clipPath: selectedProject === index ? "inset(0 0 0)" : "inset(50% 0 50%)",
                    }}
                >
                    <p className="text-[#010101] font-medium mb-3 leading-[6.5vw] text-[7.5vw] relative z-1">
