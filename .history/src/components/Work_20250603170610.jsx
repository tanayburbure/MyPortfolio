...
function Title({ project, index, setSelectedProject }) {
    ...
    return (
        <div className="relative">
            ...
            >
                <div className="inline-block mt-[-1vh] pb-1 pl-[15%] relative">
                    {/* Dark text (base layer) */}
                    <p className="text-[#2F2D29] p-2 font-medium text-[8.2vw] leading-[6.5vw]">
                        {project.title}
                    </p>
                    
                    <motion.p
                        style={{ clipPath: clipPathStyle }}
                        className="absolute top-0 left-[14.85vw] text-[#b7ab98] p-2 font-medium text-[8.2vw] leading-[6.5vw]"
                    >
                        {project.title}
                    </motion.p>
                </div>
...
function Descriptions({ projects, selectedProject }) {
    return (
        <div className="absolute top-0 w-screen z-2 pointer-events-none">
            {projects.map((project, index) => (
                <div
                    key={index}
                    className="bg-[#ec4e39] h-[7vw] text-[#0D0D0D] font-medium flex justify-between items-center px-[15.5%] transition-all duration-500 ease-in-out"
                    style={{
                        clipPath: selectedProject === index ? "inset(0 0 0)" : "inset(50% 0 50%)"
                    }}
                >
                    <p className="text-[#010101] p-2 font-medium text-[8vw] relative z-1">
                        {project.title.substring(0, 8)}
                    </p>
                    <p className="w-[40%] text-[1vw] p-2 font-bold">{project.description}</p>
                </div>
            ))}
        </div>
    );
