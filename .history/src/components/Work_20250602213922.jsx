...
const projects = [
    ...
    { title: "VISUAL", description: "Future of UFC Sports Ecosystem despite not being a sports fan.", speed: 0.5 },
    { title: "MOTION", description: "Visual concept and design language for the Lincoln Zephyr 2022.", speed: 0.67 },
    { title: "PRODUCT", description: "Part of a massive team that created a Royal Caribbean eco-system.", speed: 0.8 },
    { title: "TUTORIAL", description: "Designed a 1M+ users product utilizing my best experience: sleeping.", speed: 0.8 }
];
...
export default function Projects() {
    ...
    return (
        <>
            {/* Add the h5 tag here, outside the Projects container */}
            <h5 className="z-50 top-0 pb-4 pl-[15.5vw] shadow-lg">
                W H A T &nbsp; I &nbsp; D O
            </h5>
...
function Title({ project, index, setSelectedProject }) {
    ...
    return (
        <div className="relative">
            ...
            >
                <div className="inline-block pl-[15%]">
                    <motion.p
                        style={{ clipPath: clipPathStyle }}
                        className="text-[#b7ab98] mt-2 uppercase font-medium text-[8vw] leading-[6.5vw] relative z-2"
                    >
                        {project.title}
                    </motion.p>
                    <p className="absolute top-0 text-[#0D0D0D]">{project.title}</p>
                </div>
            </div>
        </div>
    );
    ...
    return (
        <div className="absolute top-0 h-full w-full z-2 pointer-events-none">
            {projects.map((project, index) => (
                <div
                    key={index}
                    className="bg-[#ec4e39] text-[#0D0D0D] font-medium flex justify-between items-center px-[15%] transition-all duration-500 ease-in-out"
                    style={{
                        clipPath: selectedProject === index ? "inset(0 0 0)" : "inset(50% 0 50%)"
                    }}
                >
                    <p className="text-[#010101] mb-2 uppercase font-medium text-[8vw] leading-[7.1vw] relative z-1">
