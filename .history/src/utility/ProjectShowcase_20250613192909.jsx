...
export default function ProjectShowcase({ projects }) {
    ...
    // Ensures accurate width detection even before paint
    useLayoutEffect(() => {
        const checkMobile = () => {
...
function Title({ project, index, setSelectedProject, isMobile }) {
    ...
    const handleTouchStart = () => {
        if (window.innerWidth < 768) setSelectedProject(index);
    };

    const handleTouchEnd = () => {
        if (window.innerWidth < 768) setTimeout(() => setSelectedProject(null), 1000);
    };
    ...
    const handleMouseLeave = () => {
        if (window.innerWidth >= 768) setSelectedProject(null);
    };
